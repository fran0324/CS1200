...
def bfs_2_coloring(G, precolored_nodes=None):
    ...
    # If there is no valid coloring, reset all the colors to None using G.reset_colors()
        # Check that the precolored nodes form an independent set
    if precolored_nodes is not None:
        precolored_set = set(precolored_nodes)
        for u in precolored_set:
            for v in G.edges[u]:
                if v in precolored_set:
                    G.reset_colors()
                    return None

    # Run BFS on each connected component of the graph induced by
    # the non-precolored vertices
    for v0 in range(G.N):
        if v0 in visited:
            continue

        F = {v0}   # current frontier
        S = {v0}   # discovered vertices in this component
        d = 0
        ...
        while len(F) > 0:
            ...
            # Build the next frontier F_{d+1}
            for u in F:
                for w in G.edges[u]:
                    if w in visited:
                        continue
            ...
            # Assign colors to vertices in F_{d+1}
            next_color = 1 if d % 2 == 0 else 0
            for w in new_F:
                G.colors[w] = next_color

            # Check for same-color edges among discovered vertices
            for u in F:
                for w in G.edges[u]:
                    if w in S and G.colors[w] == G.colors[u]:
                        G.reset_colors()
                        return None
